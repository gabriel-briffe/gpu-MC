...
pub const IDW_NEIGHBORS: usize = 8;
...
pub fn build_weight_table(lats: &[f32], lons: &[f32], spacing_deg: f32) -> Result<StoredWeightTable, String> {
    if lats.len() != lons.len() {
        return Err("lats and lons must have the same length".into());
    }

    let bounds = compute_bounds(lats, lons)?;
...
pub fn apply_weight_table(table: &StoredWeightTable, values: &[f32]) -> Result<Vec<f32>, String> {
    let neighbor_count = table.neighbor_count as usize;
    if neighbor_count == 0 {
        return Err("neighbor_count must be > 0".into());
    }
    if table.indices.len() % neighbor_count != 0 {
        return Err("indices length must be a multiple of neighbor_count".into());
    }
    if table.weights.len() != table.indices.len() {
        return Err("weights length must match indices".into());
    }

    let cell_count = table.indices.len() / neighbor_count;
    ...
    for k in 0..cell_count {
        ...
        for n in 0..neighbor_count {
            let p = table.indices[base + n];
            if p == INVALID_INDEX {
                continue;
            }
            let weight = f64::from(table.weights[base + n]);
            if weight <= 0.0 {
                continue;
            }
            let p = p as usize;
