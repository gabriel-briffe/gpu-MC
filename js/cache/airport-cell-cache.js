/** Per-cell OpenAIP airport cache (Cache API) — 1 week TTL. */

export const AIRPORT_CELL_TTL_MS = 7 * 24 * 60 * 60 * 1000;
const CACHE_NAME = "gpu-mc-airport-cells-v1";
...
function cellRequest(cellKey) {
  return new Request(
    `https://gpu-mc.local/airport-cell/${encodeURIComponent(String(cellKey))}`
  );
}

/**
 * @returns {{ airports: object[], fetchedAt: number } | null}
 */
export async function getCachedAirportsForCell(cellKey, now = Date.now()) {
  ...
  try {
    ...
    if (!Number.isFinite(fetchedAt) || now - fetchedAt >= AIRPORT_CELL_TTL_MS) {
    ...
    return { airports: data.airports, fetchedAt };
  } catch (error) {
    console.warn("Airport cell cache read failed", cellKey, error);
    return null;
  }
}
...
export async function putCachedAirportsForCell(cellKey, airports, fetchedAt = Date.now()) {
  ...
  try {
    ...
    console.warn("Airport cell cache write failed", cellKey, error);
    return false;
  }
}
...
export async function deleteCachedAirportsForCell(cellKey) {
  ...
  try {
    ...
    console.warn("Airport cell cache delete failed", cellKey, error);
  }
}
...
export async function pruneAirportCellCacheExcept(keepCellKeys, now = Date.now()) {
  ...
  try {
    ...
    for (const request of keys) {
      ...
      const match = url.match(/\/airport-cell\/([^/?#]+)/);
    ...
    console.warn("Airport cell cache prune failed", error);
  }
  return { removed };
}
...
export function isAirportCellCacheFresh(fetchedAt, now = Date.now()) {
  return Number.isFinite(fetchedAt) && now - fetchedAt < AIRPORT_CELL_TTL_MS;
}
